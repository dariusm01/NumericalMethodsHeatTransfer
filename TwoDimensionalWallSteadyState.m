...
while iter < iterLimit
    ...
    for m = 2:yNodes-1 % y is the rows 
        ...
        % Top
        temps(1,n) = ((2*h3*(dx^2)*dy*Tinf3) + (k*(dy^2)*(T(m,n-1) + T(m,n+1))) +...
            (2*k*(dx^2)*T(m+1,n)) + (egen*(dx^2)*(dy^2)))/((2*h3*(dx^2)*dy) + (2*k*(dy^2)) +...
            (2*k*(dx^2)));

        % Bottom
        temps(end,n) = ((2*h4*(dx^2)*dy*Tinf4) + (k*(dy^2)*(T(m,n-1) + T(m,n+1))) +...
            (2*k*(dx^2)*T(m-1,n)) + (egen*(dx^2)*(dy^2)))/((2*h4*(dx^2)*dy) + (2*k*(dy^2)) +...
            (2*k*(dx^2)));

        % Left
        temps(m,1) = ((2*h1*(dy^2)*dx*Tinf1) + (k*(dx^2)*(T(m-1,n) + T(m+1,n))) +...
            (2*k*(dy^2)*T(m,n+1)) + (egen*(dx^2)*(dy^2)))/((2*h1*(dy^2)*dx) + (2*k*(dx^2)) +...
            (2*k*(dy^2)));
