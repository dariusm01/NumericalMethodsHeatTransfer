...
while iter < iterLimit
    ...
    for m = 2:yNodes-1 
        ...
        % Top
        temps(1,2:xNodes-1) = ((h3*(dx^2)*dy*Tinf3) + (2*k*(dy^2)*(T(m-1,n) + T(m+1,n))) +...
            (2*k*(dx^2)*T(m,n+1)) + (egen*(dx^2)*(dy^2)))/((h3*(dx^2)*dy) + (4*k*(dy^2)) +...
            (2*k*(dx^2)));

        % Bottom
        temps(end,2:xNodes-1) = ((h4*(dx^2)*dy*Tinf4) + (2*k*(dy^2)*(T(m-1,n) + T(m+1,n))) +...
            (2*k*(dx^2)*T(m,n-1)) + (egen*(dx^2)*(dy^2)))/((h4*(dx^2)*dy) + (4*k*(dy^2)) +...
            (2*k*(dx^2)));


        % Left
        temps(2:yNodes-1,1) = ((h1*(dy^2)*dx*Tinf1) + (2*k*(dx^2)*(T(m,n-1) + T(m,n+1))) +...
            (2*k*(dy^2)*T(m+1,n)) + (egen*(dx^2)*(dy^2)))/((h1*(dy^2)*dx) + (4*k*(dx^2)) +...
            (2*k*(dy^2)));

        % Right
        temps(2:yNodes-1,end) = ((h2*(dy^2)*dx*Tinf2) + (2*k*(dx^2)*(T(m,n-1) + T(m,n+1))) +...
